...
p and 63
170 sx=int(rnd(1)*gx+1):sy=int(rnd(1)*gy
+1):if b(sx,sy)<>oc then 170
180 b(sx,sy)=sp:poke ul+(sx-1)*2+40*sy,s
p and 63
...
d yx<>1)
250 c=b(yx,yy)
260 b(yx,yy)=yp:poke ul+(yx-1)*2+40*yy,
yp and 63
270 if c=sp then 500
280 if c=ib then 600
290 rem =-=-=update enemy ship=-=-=
300 b(sx,sy)=oc:poke ul+(sx-1)*2+40*sy,o
c
310 sx=sx+sgn(yx-sx):sy=sy+sgn(yy-sy)
320 c=b(sx,sy)
330 b(sx,sy)=sp:poke ul+(sx-1)*2+40*sy,
sp and 63
...
erg."
620 goto 800
700 print"you are safe - enemy vessel st
ruck an iceberg"
800 stop
1000 print"S";
1010 print"."
